...
import shop.mtcoding.querydsl.dto.QItemDTO;
...
import static shop.mtcoding.querydsl.entity.QItemEntity.itemEntity;
import static shop.mtcoding.querydsl.entity.QProductKindEntity.productKindEntity;
import static shop.mtcoding.querydsl.entity.QProductMakerEntity.productMakerEntity;
        ...
        JPAQuery<ItemDTO> query = jpaQueryFactory
                .select(new QItemDTO( // DTO로 리턴
                    ...
                    JPAExpressions // 스칼라 서브쿼리
                        ...
                        .where(productKindEntity.id.eq(itemEntity.productKind)),
                    productMakerEntity.makerName,
                    itemEntity.stock
                ))
                .from(itemEntity) // 기준 테이블
                .join(productMakerEntity).on(itemEntity.productMaker.eq(productMakerEntity.id)) // 조인
                .where(customSearch(searchType, searchValue)) // 동적 조건을 위한 함수호출
                .orderBy(itemEntity.id.desc()); // 역정렬
